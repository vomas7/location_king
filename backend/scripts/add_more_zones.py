    ...
    async with async_session() as session:
        try:
            ...
            for zone_data in ADDITIONAL_ZONES:
                ...
                existing_stmt = select(LocationZone).where(LocationZone.name == zone_data["name"])
                ...
                added_count += 1
                logger.info(
                    f"Added zone: {zone_data['name']} (difficulty: {zone_data['difficulty']})"
                )
            ...
            stats_stmt = (
                ...
                .where(LocationZone.is_active == True)
            ...
            for category, count, avg_diff in stats:
                logger.info(
                    f"{category or 'No category'}: {count} zones, avg difficulty: {avg_diff:.1f}"
                )
