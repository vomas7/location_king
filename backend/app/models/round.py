...
class Round(Base):
    ...
    # Метаданные
    satellite_image_url: Mapped[str | None] = mapped_column(Text)
    ...
    def submit_guess(self, guess_lng: float, guess_lat: float) -> None:
        ...
        self.distance_km = Decimal(
            str(
                CoordinateUtils.calculate_distance_haversine(
                    guess_lng,
                    guess_lat,
                    target_coords[0],
                    target_coords[1],
    ...
    def get_guess_coordinates(self) -> tuple[float, float] | None:
