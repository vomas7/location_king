...
Места берутся из каталога `backend/scripts/seed.py`: города, побережья и
известные городские объекты. Дикой местности в нём нет намеренно — лес, песок
и открытая вода выглядят на снимке однородным ковром, и определить их нельзя
никаким усилием. Радиус зоны ограничен десятью километрами, чтобы точка раунда
не уходила из застройки; это проверяется тестом.
...
| `GET`  | `/api/zones`                            | зоны с фильтрами по месту                   |
