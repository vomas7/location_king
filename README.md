...
По умолчанию контур собран под Cloudflare: браузер идёт к Cloudflare, тот к
серверу — по origin-сертификату, а настоящий адрес игрока приходит заголовком
`CF-Connecting-IP`. Список сетей Cloudflare и клиентский сертификат для
Authenticated Origin Pulls `deploy.sh` подтягивает сам.
